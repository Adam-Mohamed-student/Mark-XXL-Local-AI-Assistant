...
def _bootstrap() -> None:
    ...
    if not need:
        return
    print(f"\n[MARK XXL] First-run setup — installing: {', '.join(need)}")
    print("[MARK XXL] This happens only once.\n")
    _sp.run([_sys.executable, "-m", "pip", "install", *need], check=True)
    print("\n[MARK XXL] Base packages ready — restarting…\n")
